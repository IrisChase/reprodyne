# Reprodyne
Generally speaking, real world data is both easier and faster to generate, and more useful to test against, than artificial test conditions written in sterile environments. Reprodyne is an Apache 2 licensed C library, written in C++, designed to capture real data generated by manual testing and integrate it into automated test suites.
...
The Reprodyne API is defined entirely as a set of preprocessor macros, so they gracefully expand into no-ops and so that there is no need to link to it after you're done testing.
